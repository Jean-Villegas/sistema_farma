        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
            ...
            {visibleNav.map((item) => (
              <button
                ...
                className={`px-3 py-2 rounded-xl text-sm font-semibold transition-colors ${
                  currentView === item.id ? 'bg-sky-100 text-sky-700' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative group">
              <button className="w-9 h-9 rounded-full bg-gradient-to-br from-sky-500 to-cyan-600 text-white text-sm font-bold flex items-center justify-center shadow-sm">
              ...
              <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                ...
                <button onClick={() => navigate('perfil')} className="w-full text-left px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-2">
          ...
          {visibleNav.map((item) => (
            <button
              ...
              className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                currentView === item.id ? 'bg-sky-100 text-sky-700' : 'text-slate-500 hover:bg-slate-100'
              }`}
            >
